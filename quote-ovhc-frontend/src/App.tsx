...
import { OVHLogin } from './components/OVHLogin';
...
const App: React.FC = () => {
    const { isAuthenticated, isLoading, logout, user, isAdmin } = useOVHAuth();
    ...
    // Use the new hooks for optimized quote fetching (enable for authenticated users only)
    const { quote: optimizedQuote, isLoading: quoteLoading, updateQuote } = 
        useQuote(isAuthenticated ? currentQuoteId : null, { enableOptimisticUpdates: isAuthenticated });
    ...
    // Fetch quote for unauthenticated users when they sign out
    useEffect(() => {
        if (!isAuthenticated && !quote && !loading) {
            // User is not authenticated and has no quote, fetch one
            fetchFirstQuote();
        }
    }, [isAuthenticated, quote, loading]);
    ...
    useEffect(() => {
        if (isAuthenticated && user) {
            ...
            // Set user email and username from auth context
    ...
    const fetchNextQuote = async (): Promise<void> => {
        ...
        try {
            setLoading(true);
            // Double-check authentication status to prevent race conditions
            if (isAuthenticated && user) {
                // For authenticated users, get next sequential quote
        ...
        try {
            if (displayQuote && !displayQuote.liked) {
                ...
                if (updatedQuote && updatedQuote.id === displayQuote.id) {
                    ...
                    // Update local state and cache for authenticated users
                    if (isAuthenticated) {
                        setQuote({ ...displayQuote, liked: true });
