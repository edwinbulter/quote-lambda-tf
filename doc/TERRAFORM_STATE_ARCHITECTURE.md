...
1. **Create shared bootstrap** (one-time):
   ```bash
   cd quote-lambda-tf-frontend/infrastructure/bootstrap/
...
2. **Migrate frontend**:
   ```bash
   cd ../
   aws s3 cp s3://quote-lambda-tf-frontend-terraform-state/quote-lambda-tf-frontend/terraform.tfstate \
             s3://edwinbulter-terraform-state/quote-lambda-tf-frontend/terraform.tfstate
   terraform init -reconfigure
   terraform state rm aws_s3_bucket.terraform_state aws_s3_bucket_versioning.terraform_state \
...
3. **Migrate backend**:
   ```bash
   cd ../../quote-lambda-tf-backend/infrastructure/
   aws s3 cp s3://quote-lambda-tf-backend-terraform-state/quote-lambda-tf-backend/terraform.tfstate \
